...
def convert_line_to_example(line):
    ...
    indices = [index[w] for w in words
                  if w in index]
    # Take the mean of the embeddings
    #x = np.zeros(num_words) # np.mean(np.vstack(embeddings), axis=0)
    #for ind in indices:
    #    x[ind] += 1
    #x = x / len(indices)
    return {'indices': indices, 'y': y, 'w':None}

def indices_to_mean_one_hot(ind_dict):
    # gc.collect()
    indices = ind_dict['indices']
    y = ind_dict['y']
    w = ind_dict['w']
    x = np.zeros(num_words) # np.mean(np.vstack(embeddings), axis=0)
    for ind in indices:
...
with open("../../lectures/Data/movie-pang02.txt", "r",encoding=enc) as f:
    # dataset = [l for l in f.readlines()]
...
for epoch in range(250):
    for batch in range(train_batches):
        data =[indices_to_mean_one_hot(l) for l in train[batch*batch_size:(batch+1)*batch_size] ]
...
test_batches = 1 * total_batches // 4
acc = 0.0
for batch in range(test_batches):
    data =[indices_to_mean_one_hot(l) for l in test[batch*batch_size:(batch+1)*batch_size] ]
    test_reviews = [sample['x'] for sample in data]
    test_labels  = [sample['y'] for sample in data]
    test_labels = np.array(test_labels).reshape([-1, 1])
    acc = (batch*acc + sess.run(accuracy, feed_dict={X: test_reviews, y: test_labels}))/(batch+1.0)
