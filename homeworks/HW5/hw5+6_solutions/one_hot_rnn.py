...
def indices_to_one_hots(ind_dict):
    indices = ind_dict['indices']
    y = ind_dict['y']
    w = np.zeros((len(indices), num_words))
    w[np.arange(len(indices)), indices] = 1.0
    return {'x': None, 'y': y, 'w': w}
...
for epoch in range(2):
    for batch in range(train_batches):
        ...
        reviews = np.array([indices_to_one_hots(sample)['w'] for sample in data]).reshape([batch_size,-1,num_words])
...
n=0
for sample in test:
    # test_reviews = np.array([sample['w'] ]).reshape([1,-1,num_words])
    test_reviews = np.array([ indices_to_one_hots(sample)['w'] ]).reshape([1,-1,num_words])
